...
static bool infrared_signal_is_message_valid(const InfraredMessage* message) {
    ...
    uint32_t address_mask = address_length >= 32 ? UINT32_MAX : (1UL << address_length) - 1;
    ...
    uint32_t command_mask = command_length >= 32 ? UINT32_MAX : (1UL << command_length) - 1;
...
static inline InfraredErrorCode
    ...
    do {
        ...
        if(!timings_size || timings_size > MAX_TIMINGS_AMOUNT) {
